...
점검 결과는 목표 대비 분량, 군더더기 표현 후보, 반복 단어, 긴 문장과 구조 보완 항목을 구분해서 보여줍니다. 결과는 하나의 불투명한 점수로 축약하지 않고 수정 순서와 함께 제공합니다. 사용자는 원고와 결과를 복사하고 원고 작성 단계로 돌아가 다시 점검할 수 있습니다.

구조 점검은 의미를 판정하지 않고 역할, 경험, 결과와 마무리를 나타내는 문장 단서만 찾습니다. 확인된 항목에는 감지 근거 문장을 함께 표시하고, 단서가 없으면 보완 질문을 제공합니다.
...
Guide 상세는 화면에 실제 작성자를 표시하고 Article JSON-LD의 작성자와 일치시킵니다. 본문을 읽은 뒤 사용할 수 있는 관련 Tool 또는 프로그램과 태그가 가까운 Guide를 제공해 다음 행동 없이 끝나는 페이지를 만들지 않습니다.
...
Tool 상세는 입력 UI만 제공하지 않고 사용 순서, 계산 기준, 입력과 결과 예시, 한계와 관련 프로그램 또는 Guide를 함께 설명합니다. 계산에 사용한 기준과 실제 결과가 달라질 수 있는 조건을 사용자가 확인할 수 있어야 합니다.
...
- 사이트 확인 스크립트는 모든 공개 route의 HTML `<head>`에 한 번만 렌더링합니다.
