...
- `/start`, `/about`, `/founder`, `/program`, `/privacy`, `/contact`, `/guides` 화면을 유지합니다.
...
- Home, 시작하기, 회사소개, 대표소개, 프로그램 소개, 개인정보 처리방침, 문의, 404 화면에는 초기 광고를 게재하지 않습니다.
...
Guides는 Naketing 주제에 맞는 검색 유입과 광고 게재 기반을 만드는 원본 콘텐츠입니다. 목록, 정적 상세 경로와 Markdown 파싱이 구현됐으며 원본 Guide 7개가 등록돼 있습니다. AdSense 신청 전 내부 기준인 10개까지 콘텐츠 확장을 진행합니다.
...
무료 Tools는 Naketing의 말하기와 자기표현 주제에 직접 연결되어야 하며, 초기에는 서버가 필요 없는 기능을 브라우저 내부에서 처리합니다. 우선 구현 후보는 다음과 같습니다.
