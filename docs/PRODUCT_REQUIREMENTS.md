...
사용 상황은 면접, 발표, 네트워킹, 일반 자기소개이고 강조할 내용은 강점, 경험, 성과, 지원 동기입니다. 목표 시간은 30초, 1분, 3분이며 발화 속도는 느리게, 보통, 빠르게를 제공합니다. 첫 진입 기본값은 면접, 경험, 1분, 보통 속도입니다.
...
원고 작성 단계는 자유 입력과 질문별 작성 도우미를 함께 제공합니다. 작성 도우미는 사용자가 입력한 역할, 상황별 경험, 강조 근거와 마무리 답변만 순서대로 연결하며 내용을 생성하거나 기존 원고를 자동으로 덮어쓰지 않습니다.

첫 점검 후 원고를 수정해 다시 점검하면 첫 결과를 기준으로 글자 수, 예상 시간, 후보 표현, 긴 문장과 구조 단서의 변화를 보여줍니다. 기준 결과는 브라우저 메모리에서만 유지하며 새로 시작하면 삭제합니다. 개선을 불투명한 종합 점수로 표현하지 않습니다.
...
- 키보드만으로 모든 단계를 완료할 수 있어야 하며 단계 전환 시 새 제목으로, 입력 오류 시 해당 입력으로 포커스를 이동합니다. 오류 메시지는 관련 입력과 접근성 속성으로 연결합니다.
- `/methodology`는 발화 속도, 목표 분량 허용 범위, 후보 표현, 반복 단어, 긴 문장, 구조 단서, 수정 전후 비교와 해석 한계를 사용자에게 공개하는 기준 문서입니다. 계산 규칙을 바꾸면 이 화면도 함께 갱신합니다.
