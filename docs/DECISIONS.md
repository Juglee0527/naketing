...
## D-007: Naketing 주요 메뉴를 세 영역으로 제한
...
시작하기, 회사소개와 대표소개가 별도 화면으로 나뉘어 주요 콘텐츠인 프로그램, 가이드와 도구의 탐색을 방해했습니다.

### 결정

Home은 서비스의 진입점으로 유지하고 Header의 주요 메뉴는 프로그램 소개, 가이드, 도구 3개만 제공합니다. 개인정보 처리방침과 문의는 광고 운영에 필요한 보조 정보이므로 Footer에서만 연결합니다.

### 결과

- `/start`, `/about`, `/founder` route와 sitemap 항목을 제거합니다.
- 프로그램 소개는 서비스 방향, 가이드는 원본 콘텐츠, 도구는 브라우저 기반 기능을 각각 담당합니다.
- 새 주요 메뉴를 추가하려면 세 영역으로 해결할 수 없는 별도 사용자 목적이 있는지 먼저 검토합니다.

## D-008: 프로그램 소개를 브라우저 기반 자기소개 점검 흐름으로 전환
...
프로그램 방향만 안내하는 화면은 사용자가 완료할 수 있는 행동이 없고 서비스가 아직 준비 중이라는 인상을 줄 수 있습니다. 기존 말하기 시간 계산 로직과 정적 Guide는 서버 없이도 자기소개 작성과 점검을 연결할 기반을 제공합니다.

### 결정

`/program`에서 상황과 목표 설정, 원고 작성, 규칙 기반 점검의 3단계 흐름을 제공합니다. 원고는 브라우저 메모리에서만 처리하고 로그인, 저장, 외부 API와 AI 분석은 사용하지 않습니다.

### 결과

- 프로그램은 준비 중 기능이 아니라 현재 사용 가능한 대표 사용자 흐름입니다.
- 계산 결과는 확인 가능한 글자 수, 예상 시간과 표현 규칙으로 설명합니다.
- 음성의 품질, 원고의 사실관계나 의미를 평가한 것처럼 표현하지 않습니다.
- 질문별 작성 도우미는 사용자의 답만 결합하고 기존 원고를 자동으로 덮어쓰지 않습니다.
- 수정 전후 비교는 첫 점검을 브라우저 메모리에만 보관하며 종합 점수를 만들지 않습니다.
- 공개된 판정 기준과 한계는 `/methodology`에서 확인할 수 있어야 하며 규칙 변경 시 함께 갱신합니다.
